...
apply_yaml_files() {
  ...
  for key in GITHUB_APP_APP_ID GITHUB_APP_CLIENT_ID; do
    echo "debug point: key: $key: ${!key} "
...
main() {
  ...
  helm upgrade -i ${RELEASE_NAME} -n ${NAME_SPACE} $HELM_REPO_NAME/$HELM_CHART_NAME --version ${CHART_VERSION} -f $DIR/value_files/${HELM_CHART_VALUE_FILE_NAME} --set global.clusterRouterBase=${K8S_CLUSTER_ROUTER_BASE} --set upstream.backstage.image.tag=${TAG_NAME}
